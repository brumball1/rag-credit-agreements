...
def log_zipf_plot_raw(top_n=500):
    ...
    plt.tight_layout()
    plt.savefig("data/derived/figures/log_zipf_raw_top500.png", dpi=600)
    plt.show()


def log_zipf_plot_lemmatised(top_n=500):
    ...
    plt.grid(True, which="both", linestyle="--", alpha=0.7)
    plt.tight_layout()
    plt.savefig("data/derived/figures/log_zipf_lemma_top500.png", dpi=600)
    plt.show()
...
def zipf_plot_raw(top_n=500):
    ...
    ranks = np.arange(1, top_n + 1)
    plt.figure(figsize=(8, 6))
    plt.plot(ranks, freqs, marker=".", linestyle="none")
    plt.title("Zipf Plot for Raw Token Frequencies")
    plt.xlabel("Rank")
    plt.ylabel("Frequency")
    plt.grid(True, which="both", linestyle="--", alpha=0.7)
